...
const ListCreate = ({ activeTab }) => {
  ...
  // 🔴 เปลี่ยนให้ดึงข้อมูลจาก API ใหม่ของ Main Format
  const handleMergeData = async () => {
    try {
      setStep('generating');
      // ยิงไปที่ /preview-main แทน /merge เดิม
      const response = await fetch('http://localhost:3000/api/part-list/preview-main');
  ...
  // 🔴 เอา Header ของทั้ง 27 คอลัมน์มาแสดงทั้งหมด ไม่ต้องตัด (Slice) ออกแล้ว
          ...
          {(step === 'idle' || step === 'generating') && (
            <div className="flex flex-col items-center gap-8">
              ...
              {uploadStatus.target && uploadStatus.proc && step !== 'generating' && (
                <button onClick={handleMergeData} className="bg-dark text-white px-10 py-4 rounded-full font-bold shadow-xl shadow-dark/20 hover:bg-primary transition-all flex items-center gap-2 animate-in zoom-in">
          ...
          {step === 'preview' && (
            <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4">
              ...
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 className="font-bold text-dark">Main Format Preview ({previewData.length} items)</h3>
                <button onClick={() => { setStep('idle'); setUploadStatus({target:false, proc:false}); setPreviewData([]); }} className="text-xs bg-gray-200 px-4 py-2 rounded-full font-bold text-dark hover:bg-gray-300 transition-colors">
                  Reset & Upload New
                </button>
              </div>

              {/* 🔴 ตารางโชว์ครบ 27 คอลัมน์ (Scroll ซ้ายขวาได้) */}
              <div className="overflow-x-auto max-w-full max-h-[400px]">
                ...
                {previewData.length > 50 && (
                  <div className="text-center py-3 text-xs text-gray-400 bg-gray-50/30 border-t border-gray-100">
              ...
              <div className="p-6 bg-gray-50/80 flex justify-end border-t border-gray-100">
                <button onClick={() => setIsModalOpen(true)} className="bg-dark text-white px-8 py-3 rounded-xl font-bold shadow-md hover:bg-black transition-colors flex items-center gap-2">
